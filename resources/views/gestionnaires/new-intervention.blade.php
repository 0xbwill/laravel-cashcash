<x-app-layout>
    <div class="container mt-5">
        <div class="row d-flex justify-center">
            <div class="col-md-5 mx-2 rounded-lg h-100 pb-4 px-4 "
                ...
                <form action="{{ route('insert.intervention') }}" method="POST" class="text-white form-group">
                    ...
                    <select class="form-control mb-4" name="numeroIntervenant" id="numeroIntervenant">
                        {{-- Liste déroulante pour selectionner un intervenant de la même agence que les clients --}}
                        <option value="">Sélectionnez un intervenant</option>
                        @foreach ($intervenants as $unIntervant)
                            <option value="{{ $unIntervant->id }}">
                                {{ $unIntervant->id . ' | ' . $unIntervant->name . ' ' . $unIntervant->surname }}</option>
                        @endforeach
                    </select>
                        {{-- Permet de selectionner une date --}}
                        <label for="dateVisite">Date visite :</label>
                        <input class="form-control mb-4" type="date" name="dateVisite" id="dateVisite">
                        {{-- Permet de selectionner une heure --}}
                    ...
                    {{-- Bouton permettant de confirmer l'assignation d'une nouvelle interventions  --}}
            ...
            <div class="col-md-5 mx-5 rounded-lg h-100 pb-3 text-white px-4 pb-4"
                style="border: 2px #282f3d solid;background-color: #0e131c">
                {{-- Affiche des informations du client à droite sur la page --}}
